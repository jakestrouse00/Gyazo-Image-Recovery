...
def getImages(session):
    ...
    if r.reason == 'Forbidden':
        ...
        for element in r.json():
            ...
            imageID = element['alias_id']
            s = [imageApp, imageTitle, imageURL, imageType, imageID]
            images.append(s)
        return images
...
while True:
    ...
    if images is not None:
        ...
        for image in images:
            ...
            if image[4] not in currentIDs:
                # sort the image into it's folders
                sortImages(image)
                # add the new image ID to the list to make sure it won't be downloaded again
                currentIDs.append(image[4])
    # pause the program for 60 seconds before checking for new images
    time.sleep(60)
    print("\nCHECKING FOR MORE IMAGES/GIFS\n")
